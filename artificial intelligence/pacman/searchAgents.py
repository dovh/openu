...
class CornersProblem(search.SearchProblem):
  ...
  def getSuccessors(self, state):
    ...
    successors = []
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      dx, dy = Actions.directionToVector(action, 1)
      nextx, nexty = int(x + dx), int(y + dy)
      hitsWall = self.walls[nextx][nexty]
      if hitsWall:
        continue

      nextState = ((nextx, nexty), list(state[1]))
      for i in range(4):
        if nextState[0] == self.corners[i]:
          nextState[1][i] = False
...
def cornersHeuristic(state, problem):
  ...
  # find the shortest path to reach all remanding corners using mdh 
  #   for maximum 4 iterations, each time find the closest one
  ...
  while len(corners_list) != 0:
    ...
    for corner in corners_list: 
      mhd = abs(corner[0]-position[0]) + abs(corner[1]-position[1]) 
      if mhd < min:
        min = mhd 
        closest = corner 

    corners_list.remove(closest)
    position = closest
    h += min
...
def foodHeuristic(state, problem):
  ...
  # find the shortest path to reach all remanding food using mdh 
  ...
  while len(foodList) != 0:
    ...
    for food in foodList: 
      mhd = abs(food[0]-position[0]) + abs(food[1]-position[1]) 
      if mhd < min:
        min = mhd 
        closest = food 
