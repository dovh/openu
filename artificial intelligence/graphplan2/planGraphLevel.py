...
def mutexPropositions(prop1, prop2, mutexActions):
  ...
  # in case of initial propositions
  if len(prop1.getProducers()) == 0 or len(prop2.getProducers()) == 0:
    return False
  
  for action1 in prop1.getProducers(): 
    for action2 in prop2.getProducers(): 
      if Pair(action1, action2) not in mutexActions:
        return False
